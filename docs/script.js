(function() {
	...
	const translations = {
		cs: {
			...
			"usage.basic_comment": "# Export podle názvu databáze",
			"usage.positional_comment": "# Název databáze lze uvést i jako pouhý argument",
			"usage.defaults_title": "Výchozí hodnoty připojení",
			"usage.defaults_note": "Parametry připojení se předávají nástroji <code>psql</code> jen tehdy, když je skutečně zadáte. Na vše ostatní se použijí výchozí hodnoty <code>psql</code> a standardní proměnné prostředí (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Pokud už máte prostředí nastavené pro <code>psql</code>, funguje i spuštění bez jakýchkoli parametrů.",
		...
		en: {
			...
			"usage.basic_comment": "# Export by database name",
			"usage.positional_comment": "# The database name can also be a plain positional argument",
			"usage.defaults_title": "Connection defaults",
			"usage.defaults_note": "Connection parameters are forwarded to <code>psql</code> only when you actually provide them. Everything you omit falls back to <code>psql</code>'s own defaults and the standard environment variables (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). If your environment is already set up for <code>psql</code>, running it with no arguments at all works too.",
		...
		es: {
			...
			"usage.basic_comment": "# Exportar por nombre de base de datos",
			"usage.positional_comment": "# El nombre de la base de datos también puede ser un argumento posicional",
			"usage.defaults_title": "Valores por defecto de conexión",
			"usage.defaults_note": "Los parámetros de conexión se pasan a <code>psql</code> solo cuando los indicas. Todo lo que omitas usa los valores por defecto de <code>psql</code> y las variables de entorno estándar (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Si tu entorno ya está configurado para <code>psql</code>, también funciona ejecutarlo sin ningún argumento.",
		...
		fr: {
			...
			"usage.basic_comment": "# Export par nom de base",
			"usage.positional_comment": "# Le nom de la base peut aussi être un simple argument positionnel",
			"usage.defaults_title": "Valeurs par défaut de connexion",
			"usage.defaults_note": "Les paramètres de connexion ne sont transmis à <code>psql</code> que si vous les fournissez. Tout ce que vous omettez utilise les valeurs par défaut de <code>psql</code> et les variables d'environnement standard (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Si votre environnement est déjà configuré pour <code>psql</code>, l'exécution sans aucun argument fonctionne aussi.",
		...
		de: {
			...
			"usage.basic_comment": "# Export nach Datenbankname",
			"usage.positional_comment": "# Der Datenbankname kann auch als einfaches Argument angegeben werden",
			"usage.defaults_title": "Verbindungs-Standardwerte",
			"usage.defaults_note": "Verbindungsparameter werden nur dann an <code>psql</code> übergeben, wenn Sie sie angeben. Für alles Weggelassene gelten die Standardwerte von <code>psql</code> und die üblichen Umgebungsvariablen (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Wenn Ihre Umgebung bereits für <code>psql</code> eingerichtet ist, funktioniert der Aufruf auch ganz ohne Parameter.",
		...
		it: {
			...
			"usage.basic_comment": "# Esportazione per nome database",
			"usage.positional_comment": "# Il nome del database può essere anche un semplice argomento posizionale",
			"usage.defaults_title": "Valori predefiniti di connessione",
			"usage.defaults_note": "I parametri di connessione vengono passati a <code>psql</code> solo quando li specifichi. Tutto ciò che ometti usa i valori predefiniti di <code>psql</code> e le variabili d'ambiente standard (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Se il tuo ambiente è già configurato per <code>psql</code>, funziona anche l'esecuzione senza alcun argomento.",
		...
		pt: {
			...
			"usage.basic_comment": "# Exportar por nome do banco",
			"usage.positional_comment": "# O nome do banco também pode ser um argumento posicional",
			"usage.defaults_title": "Padrões de conexão",
			"usage.defaults_note": "Os parâmetros de conexão são passados ao <code>psql</code> apenas quando você os informa. Tudo o que for omitido usa os padrões do próprio <code>psql</code> e as variáveis de ambiente padrão (<code>PGHOST</code>, <code>PGPORT</code>, <code>PGUSER</code>, <code>PGDATABASE</code>). Se o seu ambiente já está configurado para o <code>psql</code>, executar sem nenhum argumento também funciona.",
